...
def build_app() -> None:
    ...
    if page == "Présentation du projet":
        ...
        if chemin_imp.exists():
            st.image(str(chemin_imp), caption="Importance des features")

        st.markdown("""
        ### Interprétation

        - Le **R²** mesure la part de variance expliquée (1.0 = parfait, 0 = inutile)
        - Le **MAE** est l'erreur moyenne en watts
        ...
        puissance_estimee = max(0, round(modele.predict(X)[0]))

        st.markdown("---")
        st.metric(label="Puissance estimée", value=f"{puissance_estimee} W")
